...
function desconectado(should_i_reconnect) {
    $(".drop").prop("disabled", true);
    $("#loginSection").css("display", "block");
    if (should_i_reconnect) {
        $("#juego-msg").html("<h2>Disconnected! Reconecting...</h2>");
        setTimeout(function(){ connect(); }, 1000);
    } else {
        $("#juego-msg").html("<h2>Disconnected!</h2>");
    }
}

function turno(quien) {
    $("#juego-msg").html("<strong>" + quien + "</strong>'s turn");
}
...
function onLogin(result, quien) {
    if (result == "ok") {
        ...
        $("#juego-msg").html("<p>Waiting for the other player...</p>");
        send({type: "muestra"})
    } else {
        alert("error: " + result)
    }
